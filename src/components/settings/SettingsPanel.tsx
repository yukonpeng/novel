    ...
    <div className="absolute inset-0 z-20 flex items-center justify-center bg-black/40">
      <div className="w-[90vw] max-w-[420px] rounded-lg border border-[var(--nr-border-color)] bg-[var(--nr-bg)] p-5 text-[var(--nr-fg)] shadow-xl">
        <div className="mb-4 flex items-center justify-between border-b border-[var(--nr-separator)] pb-3">
          ...
          <button className="rounded px-2 py-1 hover:bg-[var(--nr-hover-bg)]" onClick={onClose}>×</button>
        </div>
        <label className="mb-4 block text-sm">
          <span className="mb-1 block">主题风格</span>
          <select className="w-full rounded bg-[var(--nr-input-bg)] px-3 py-2 text-[var(--nr-input-fg)]" value={state.config.theme} onChange={(event) => void actions.updateConfig({ theme: event.target.value as typeof state.config.theme })}>
        ...
        <label className="mb-6 block text-sm">
          ...
          <input className="w-full rounded bg-[var(--nr-input-bg)] px-3 py-2 text-[var(--nr-input-fg)]" value={state.config.fontFamily} onChange={(event) => void actions.updateConfig({ fontFamily: event.target.value })} />
        </label>
        <label className="mb-6 block text-sm">
        ...
        <div className="flex justify-end">
          <button className="rounded bg-[var(--nr-button-bg)] px-4 py-2 text-sm text-[var(--nr-button-fg)]" onClick={onClose}>完成</button>
        </div>
      </div>
    </div>
  );
